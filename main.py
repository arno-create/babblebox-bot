...
def get_lobby_embed(guild_id):
    ...
    if not game:
        return discord.Embed(
            title="❌ Lobby Closed",
            description="Start a new game with `/play`.",
            color=discord.Color.red(),
        )
...
class LobbyView(TrackedView):
    ...
    async def start_callback(self, interaction, button):
        ...
        async with game["lock"]:
            ...
            if game["game_type"] == "bomb" and not bot.dictionary_ready:
...
class SpyfallVoteSelect(discord.ui.Select):
    def __init__(self, players, guild_id):
        ...
        super().__init__(placeholder="Vote for the Spy (60s)...", min_values=1, max_values=1, options=options)
        self.guild_id = guild_id
...
class SpyfallVoteView(TrackedView):
    def __init__(self, guild_id):
        super().__init__(guild_id, timeout=60)
        game = games.get(guild_id)
        if game:
...
class ResignViewEnd(TrackedView):
    def __init__(self, guild_id):
        super().__init__(guild_id, timeout=TURN_TIMEOUT_SECONDS)
...
async def _prompt_corpse_player_locked(guild_id, game, player):
    ...
    return await _prompt_dm_player_locked(
        ...
        f"**Step {step + 1} of 6**\n{CORPSE_PROMPTS[step]}\n"
        "*(60s to reply - any word works, but follow the prompt for best results!)*",
    )
    ...
    async with game["lock"]:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "Select who you think the spy is. You have **60 seconds**."
            ),
            color=discord.Color.red(),
        )
...
async def play_cmd(interaction):
    ...
    async with games_guard:
        if guild_id in games:
            return await interaction.response.send_message(
                embed=discord.Embed(
                    title="❌ Game Room is busy!",
                    description="A lobby or game is already active on this server. Use `/stop` to cancel it.",
                    color=discord.Color.red(),
                ),
                ephemeral=True,
            )
    ...
    try:
        await interaction.response.send_message(embed=get_lobby_embed(guild_id), view=view)
        message = await interaction.original_response()
        register_view(guild_id, view, message)
    except Exception:
        await cleanup_game(guild_id)
        raise
...
async def stats_cmd(interaction, user: Optional[discord.User] = None):
    target = user or interaction.user
    stats = session_stats.get(target.id)
    if not stats:
        return await interaction.response.send_message(
...
@bot.tree.command(name="leaderboard", description="View the Babblebox session leaderboard")
...
async def leaderboard_cmd(interaction, metric: app_commands.Choice[str]):
    ...
    entries = [
        value
        for value in session_stats.values()
        if value.get(metric_key, 0) > 0
    ]
    if not entries:
        return await interaction.response.send_message(
    ...
    entries.sort(key=lambda item: (item.get(metric_key, 0), item.get("wins", 0), item.get("games_played", 0)), reverse=True)
    await interaction.response.send_message(embed=build_leaderboard_embed(metric_key, labels[metric_key], entries))
...
async def stop_cmd(interaction):
    ...
    if interaction.user.id != game["host"].id and not interaction.user.guild_permissions.administrator:
...
async def on_member_remove(member):
    game = games.get(member.guild.id)
    if not game or game.get("closing"):
        return

    involved = is_player_in_game(game, member.id) or game["host"].id == member.id
    if not involved:
        return
    ...
    async with game["lock"]:
        ...
        was_player = is_player_in_game(game, member.id)
        if was_player:
        ...
        if game.get("active"):
            with contextlib.suppress(discord.HTTPException):
                await game["channel"].send(
                    f"⚠️ {member.display_name} left the server during the game. The match has been cancelled to avoid a broken state."
                )
            await cleanup_game(member.guild.id)
            return

        if was_host:
            with contextlib.suppress(discord.HTTPException):
                await game["channel"].send("⚠️ The host left the server. The lobby has been closed.")
            await cleanup_game(member.guild.id)
            return

        lobby_view = next((view for view in game["views"] if isinstance(view, LobbyView)), None)
        if lobby_view and lobby_view.message:
            lobby_view.refresh_components()
            with contextlib.suppress(discord.NotFound, discord.Forbidden, discord.HTTPException):
                await lobby_view.message.edit(embed=get_lobby_embed(member.guild.id), view=lobby_view)
...
async def on_raw_message_delete(payload):
    ...
    game = games.get(payload.guild_id)
    if not game or game.get("closing"):
        return
    ...
    async with game["lock"]:
        ...
        with contextlib.suppress(discord.HTTPException):
            await game["channel"].send(
                "⚠️ A live game panel was deleted. Closing the game to prevent a broken state."
            )
        await cleanup_game(payload.guild_id)
