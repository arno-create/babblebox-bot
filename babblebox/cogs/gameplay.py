...
class GameplayCog(commands.Cog):
    ...
    async def play_command(self, ctx: commands.Context):
        if ctx.guild is None or ctx.channel is None:
            await send_hybrid_response(ctx, "This command only works in a server.", ephemeral=True)
            return
            ...
            if guild_id in ge.games:
                await send_hybrid_response(
                    ...
                    embed=discord.Embed(
                        ...
                        description="A lobby or game is already active on this server. Use `/stop` or `bb!stop` to cancel it.",
    ...
    async def stop_command(self, ctx: commands.Context):
        ...
        if not game:
            await send_hybrid_response(ctx, "There is no active game or lobby to stop.", ephemeral=True)
            return

        if ctx.author.id != game["host"].id and not ctx.author.guild_permissions.administrator:
            await send_hybrid_response(ctx, "Only the host or an administrator can stop the game.", ephemeral=True)
            return
    ...
    async def chaoscard_command(self, ctx: commands.Context):
        ...
        game = ge.games.get(ctx.guild.id)
        if not game:
            await send_hybrid_response(ctx, "There is no active lobby right now. Start one with `/play` or `bb!play`.", ephemeral=True)
            return
        ...
        if game.get("active"):
            await send_hybrid_response(
                ctx,
                f"The active Chaos Card is **{card['label']}** - {card['description']}",
                ephemeral=True,
            )
            return
        ...
        async with game["lock"]:
            ...
            if not game or game.get("closing") or game.get("active"):
                await send_hybrid_response(ctx, "This lobby is already closed.", ephemeral=True)
                return
    ...
    async def _trigger_spyfall_vote_prefix(self, ctx: commands.Context):
        ...
        if not game or game.get("closing"):
            await send_hybrid_response(ctx, "No active Spyfall game to vote on.", ephemeral=True)
            return
        ...
        async with game["lock"]:
            ...
            if not ge.is_player_in_game(game, ctx.author.id):
                await send_hybrid_response(ctx, "You are not playing.", ephemeral=True)
                return
            if game.get("voting_active"):
                await send_hybrid_response(ctx, "A vote is already in progress.", ephemeral=True)
                return
